#!/usr/bin/env bash
#
# publish.sh — Build the Codex plugin and deploy the tree to a distribution repo
# (the repo Codex marketplaces install from).
#
...
git -C "$WORKTREE" commit -q -m "build: deploy codex plugin from monorepo@${SRC_SHA}"
...
echo "==> Deployed codex plugin to $SLUG."
