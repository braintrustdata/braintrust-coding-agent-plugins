...
CARGO_TARGET_ROOT="${CARGO_TARGET_DIR:-$REPO_ROOT/bt-daemon/target}"
...
prepare_dev_dir() {
  local path="$1"
  [[ -n "$path" ]] || fail "local development directory must not be empty"
  if [[ -e "$path" || -L "$path" ]]; then
    fail "refusing to replace existing local development directory: $path"
  fi
  mkdir -p -- "$path"
}
...
daemon_binary_path() {
  ...
  if [[ -n "$build_target" ]]; then
    printf '%s/%s/debug/bt-daemon\n' "$target_dir" "$build_target"
  else
    printf '%s/debug/bt-daemon\n' "$target_dir"
  fi
}

remove_local_auth_copy() {
  rm -f -- "$1/auth.json"
}

cleanup_local_auth() {
  remove_local_auth_copy "$GROK_HOME_DIR"
}

DAEMON_BIN="$(daemon_binary_path "$CARGO_TARGET_ROOT" "${CARGO_BUILD_TARGET:-}")"
