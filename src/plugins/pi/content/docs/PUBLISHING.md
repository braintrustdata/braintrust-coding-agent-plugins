...
`@braintrust/pi-extension` is built and released from the plugin monorepo.
Version 1 requires a compatible `bt` installation with the Pi daemon
translator. If the daemon is missing or incompatible, tracing warns once and
fails open without interrupting Pi.
...
Real releases run only through `.github/workflows/release-pi.yml`. The workflow
uses `braintrustdata/sdk-actions`, environment approval, npm trusted
