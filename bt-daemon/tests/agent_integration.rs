...
async fn opencode_session_emits_traces() {
    ...
    if world.uses_mock_ingest() {
        assert!(
            rows.iter()
                .any(|row| row_contains(row, &["braintrust.plugin.opencode", "2.0.1"])),
    ...
    if world.uses_mock_inference() && world.uses_mock_ingest() {
        let scenario = IngestScenario::new()
            .expect("OpenCode trace origin", |row| {
                row_contains(row, &["braintrust.plugin.opencode", "2.0.1"])
            })
...
async fn pi_session_emits_traces() {
    ...
    if world.uses_mock_ingest() {
        assert!(
            rows.iter()
                .any(|row| row_contains(row, &["braintrust.plugin.pi", "2.0.1"])),
