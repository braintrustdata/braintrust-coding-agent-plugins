...
fn pi_checkpoint_preserves_the_open_session_and_turn() {
    ...
    assert!(ops.iter().all(|op| !matches!(op, SpanOp::Merge(row) if row.metadata.as_ref().is_some_and(|metadata| metadata.get("total_turns").is_some()))));
}
...
fn pi_finalization_closes_missing_llm_and_tool_events() {
    ...
    assert!(ops.iter().any(|op| matches!(op, SpanOp::Insert(row) if row.span_type == SpanType::Llm && row.error.is_some())));
    assert!(ops.iter().any(|op| matches!(op, SpanOp::Insert(row) if row.span_type == SpanType::Tool && row.error.is_some())));
}
