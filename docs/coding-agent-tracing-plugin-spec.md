...
normalizing agent fields. It MUST NOT inject Braintrust authentication or route
credentials into that payload. Native payloads can themselves contain sensitive
user or tool content; normalization and any configured content redaction belong
at the daemon boundary, before journaling and translation.
...
- block until local daemon acceptance at ordering-sensitive boundaries, with a
  finite adapter-defined timeout;
- quote paths and payloads safely;
- never print or persist Braintrust authentication or route credentials;
...
Braintrust authentication credentials and route secrets MUST NOT be serializable
in an envelope or journal. They are resolved by the embedding `bt` host at
delivery time. Raw native payloads MAY contain sensitive values supplied to the
coding agent and MUST follow the private-storage, disclosure, and optional
content-redaction requirements in section 9.
...
- Capture adapters MUST be free of Braintrust authentication credentials.
- Braintrust API keys, OAuth tokens, and route secrets MUST never appear in
  plugin settings, envelopes, journals, logs, command lines, or span metadata.
  Native prompts and tool payloads can contain user-supplied secrets; integrations
  MUST document that exposure and apply configured content redaction before
  journaling.
...
- persist Braintrust authentication or route credentials;
...
| 1. Foundation         | CAP-01   | Thin fail-open live capture                 | Native activity reaches the daemon without capture-owned Braintrust credentials or trace construction, and tracing failures never interrupt the agent.                                  |
...
| 1. Foundation         | SEC-01   | Credential-free capture and routes          | Plugins and routes never carry Braintrust API keys, OAuth tokens, or route secrets; native payload sensitivity and configured content redaction are explicit.                           |
